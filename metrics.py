...
def compute_metrics(opt, pred_traj, ref_data, metrics, runner,stage):
    ...
    ref_data: [num_dist, batch_size, data_dim], torch.Tensor
    '''
    sample_size     = 1000
    dist_time       = np.linspace(0, opt.interval-1, opt.num_dist).astype(int) #we delete a distribution when LOO
    pred_idx        = np.random.choice(pred_traj.shape[0], sample_size, replace=False)
    ...
    if opt.use_ae: pred_data = opt.ae.decoder(torch.Tensor(pred_data)).detach().cpu().numpy()
